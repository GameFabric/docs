...
If your game server accepts connection information via command-line arguments or configuration files, you can use the [Game Server Wrapper](/multiplayer-servers/configure/game-server-wrapper) to template these values at startup without modifying your game server code.
...
See [Command-line Arguments](/multiplayer-servers/configure/game-server-wrapper#command-line-arguments) for available template variables.
...
If you use the [Game Server Wrapper](/multiplayer-servers/configure/game-server-wrapper), it also supports a `--local` flag for local testing.
