...
Registration (step 1) and allocation (step 2) use separate authentication tokens. For each Allocator you order, you receive a **Registry Token** for game server registration and an **Allocator Token** for calling `/allocate`. See [ALLOC_TOKEN](automatically-registering-game-servers#alloc-token-string) for details.
:::
