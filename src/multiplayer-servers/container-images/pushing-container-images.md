...
1. [Build your container image](/multiplayer-servers/container-images/building-a-container-image)
2. **Push it to the registry** (this guide)
3. [Run your game server](/multiplayer-servers/deploy/vessels)
:::
...
* **Add default Branches**: This will create two branches, "Production" and "Development", with default Image Retention Policies. Don't worry - you can always change the display names and policies later. See how to [edit a branch](editing-a-branch).
...
The `${USERNAME}` and `${PASSWORD}` in the command below are your **Service Account** credentials, not your personal login or SSO credentials. If you have not created a Service Account yet, follow the [Service Accounts guide](/multiplayer-servers/administration/service-accounts).
:::
...
1. Confirm that it belongs to a group with push permissions (e.g., `default:image-providers`). See [Service Accounts](/multiplayer-servers/administration/service-accounts) for details
1. If the problem persists, reach out via the [GameFabric Help Center](/multiplayer-servers/get-started/glossary#gamefabric-help-center) or contact your Customer Success Manager
:::
