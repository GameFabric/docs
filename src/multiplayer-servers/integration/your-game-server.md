...
For example, if your server binds locally to port `7777` but GameFabric maps it to public port `12225`, the `ports` slice will contain the public port `12225`. Use this value when registering with Steam, EOS, or any external server list.
...
If your game server doesn't use an Agones SDK, you can query the local REST endpoint:

```
GET http://localhost:${AGONES_SDK_HTTP_PORT}/gameserver
```

The response includes the public address and ports in the `status` object.
