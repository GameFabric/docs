...
See [Sidecar Containers](/multiplayer-servers/concepts/sidecars#allocation-sidecar) for an overview and [Automatically Registering Game Servers](/multiplayer-servers/integrate/server-allocation/automatically-registering-game-servers) for configuration details.
...
See [Server Allocation Overview](/multiplayer-servers/integrate/server-allocation/overview) for details on when to use the Allocator and integration options.
...
See [Using the Agones SDK](/multiplayer-servers/integrate/your-game-server) for integration details and the [Agones documentation](https://agones.dev/site/docs/) for the full reference.
...
It can have multiple [revisions](/multiplayer-servers/get-started/glossary#revision), which are essentially different versions of the same Armada, as it gets updated.
...
Armadas are part of the [Dynamic Fleets](#dynamic-fleets) category in the UI. See also [hosting model](/multiplayer-servers/concepts/hosting-models).

## ArmadaSet

An ArmadaSet is the configuration for a set of Armadas that share the same Fleet template and automatic scaling strategy.

ArmadaSets are part of the [Dynamic Fleets](#dynamic-fleets) category in the UI. See also [hosting model](/multiplayer-servers/concepts/hosting-models).
...
See also [Configuring your Cloud Provider](/multiplayer-servers/configure/cloud-provider-setup) and [Capacity Types](/multiplayer-servers/concepts/capacity-types).
...
See [Armada Replicas and Buffer](/multiplayer-servers/deploy/replicas-and-buffer#buffer-size) for configuration guidance.
...
See [Using the Agones SDK](/multiplayer-servers/integrate/your-game-server)

## Cluster

A cluster refers to a group of bare metal or cloud servers located in the same location.
In the context of GameFabric, clusters are implemented as [sites](/multiplayer-servers/get-started/glossary#site).
...
See [Dynamically configuring the buffer size](/multiplayer-servers/deploy/replicas-and-buffer#dynamically-configuring-the-buffer-size) for details.
...
See [Identifying your Hosting Model](/multiplayer-servers/concepts/hosting-models) for guidance on choosing between Dynamic Fleets and [Persistent Servers](#persistent-servers).
...
An export store is a configuration object that targets one S3-compatible bucket for continuous delivery of [audit log events](/multiplayer-servers/operate/audit-logs). Each export store has a status: **Active**, **Suspended**, or **Error**.

See [Audit log exports](/multiplayer-servers/operate/audit-log-exports) for setup and configuration instructions.
...
Formations are part of the [Persistent Servers](#persistent-servers) category in the UI. See also [hosting model](/multiplayer-servers/concepts/hosting-models).
...
See also [GameFabric Cloud](/multiplayer-servers/configure/gamefabric-cloud).
...
For access control details, see [Editing Permissions](/multiplayer-servers/administration/permissions#help-center).
...
See [Editing Permissions](/multiplayer-servers/administration/permissions#group).
...
See also [Capacity Types](/multiplayer-servers/concepts/capacity-types) and [Region](#region).
...
The observability tap exposes your GameFabric installation's [monitoring](/multiplayer-servers/operate/monitoring) data to external tooling. It provides read-only access to a Prometheus-compatible metrics API and a Loki-compatible logs API, allowing you to query metrics and logs from your own observability stack.
...
See [Observability tap](/multiplayer-servers/operate/observability-tap) for endpoint details and setup instructions.

## User

See [Editing Permissions](/multiplayer-servers/administration/permissions#user).

## Permission

See [Editing Permissions](/multiplayer-servers/administration/permissions).
...
See [Identifying your Hosting Model](/multiplayer-servers/concepts/hosting-models) for guidance on choosing between Persistent Servers and [Dynamic Fleets](#dynamic-fleets).
...
See also [Editing Permissions](/multiplayer-servers/administration/permissions).
...
See [Editing Permissions](/multiplayer-servers/administration/permissions#role).
...
See [Scheduling Strategy](/multiplayer-servers/concepts/scheduling-strategy) for configuration details.

## Secret

See [Secrets](/multiplayer-servers/configure/secrets).
...
See [Sidecar Containers](/multiplayer-servers/concepts/sidecars) for a comprehensive guide.
...
See [Vessel Shutdown Behavior](/multiplayer-servers/deploy/terminating-game-servers#vessels).
...
An account used for accessing GameFabric programmatically. For details, see the documentation in [Service Accounts](/multiplayer-servers/administration/service-accounts#managing-service-accounts).
...
- **Vessels:** `Pending`, `Scheduled`, `Created`, `Starting`, `Running`, `Terminating`, `Error`, `Suspended`. See [Vessel states](/multiplayer-servers/deploy/vessel-states).
...
GameFabric offers its own Terraform provider to interact with the platform. See the [Terraform provider](/multiplayer-servers/integrate/terraform) documentation for installation and usage instructions.
...
Vessels are part of the [Persistent Servers](#persistent-servers) category in the UI. See also [hosting model](/multiplayer-servers/concepts/hosting-models).

See [Vessel states](/multiplayer-servers/deploy/vessel-states) for a description of each state a vessel can be in.

## Wrapper

A wrapper can be used to call your game server binary in order to provide additional functionality.

We provide a wrapper called [Game Server Wrapper](/multiplayer-servers/configure/game-server-wrapper).
