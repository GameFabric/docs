...
The buffer is equally important on all [capacity types](/multiplayer-servers/concepts/capacity-types): allocation always draws from the pool of `Ready` servers, regardless of the underlying infrastructure.

For detailed guidance, including examples and input validation rules, see [Replicas and buffer size](/multiplayer-servers/deploy/replicas-and-buffer).
...
For configuration details and slider values, see [Dynamically configuring the buffer size](/multiplayer-servers/deploy/replicas-and-buffer#dynamically-configuring-the-buffer-size).
...
For a full explanation of utilization metrics, panic behavior, and configuration options, see [Scale to Zero](/multiplayer-servers/deploy/scale-to-zero).
