...
Allocation speed is independent of the underlying [capacity type](/multiplayer-servers/concepts/capacity-types). Whether game servers run on bare metal or cloud, allocation draws from the pool of `Ready` servers in the buffer. When you use the [Allocator](/multiplayer-servers/get-started/glossary#allocator), it assigns a `Ready` server instantly. The buffer is equally important on all capacity types: without enough `Ready` servers, players wait for a new server to start up.

In cloud environments, GameFabric autoscales the number of nodes to match demand (see [GameFabric Cloud](/multiplayer-servers/concepts/capacity-types#gamefabric-cloud) and [Bring Your Own Cloud (BYOC)](/multiplayer-servers/concepts/capacity-types#byoc)). This does not affect the allocation path. With the right buffer configuration, there are always enough `Ready` servers available to handle allocation requests while new nodes come online in the background.
...
When looking from a resource perspective, the Maximum Replicas can be estimated by the available resources on the Location associated with the Region Type, divided by the [Resource Requests](/multiplayer-servers/configure/resource-management#resource-requests) of the game server.
...
To avoid degraded performance with reduced CPU availability, or OOMKills when memory is overcommitted, see [Resource Limits](/multiplayer-servers/configure/resource-management#resource-limits).
