...
- Tracking [CCU (Concurrent Users)](/multiplayer-servers/integrate/your-game-server)
- Optimizing [resource requests](/multiplayer-servers/configure/resource-management) based on actual usage
...
For practical guidance on sizing CPU and memory requests and limits using these metrics, see [Resource management](/multiplayer-servers/configure/resource-management).

## Logs

Everything that the game server prints to either `stdout` or `stderr` is automatically collected and available in the [monitoring dashboards](/multiplayer-servers/operate/debugging#viewing-container-logs).
...
If your game server writes logs to files, you can use the [Game Server Wrapper's log tailing feature](/multiplayer-servers/configure/game-server-wrapper#log-tailing) to forward them to `stdout`. This makes file-based logs available in Grafana and the Vessel UI alongside your other logs.
:::

::: info Logging quotas
[Logging quotas](/multiplayer-servers/configure/quotas#logging-limitations) apply. If your game server exceeds the rate limits, log lines are dropped.
If you require different limits, contact GameFabric support.
:::

## External integration

If you want to query your metrics and logs from your own observability tools, see the [Observability tap](/multiplayer-servers/operate/observability-tap).
